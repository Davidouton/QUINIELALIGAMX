...
def get_app_bootstrap(
    ...
    return AppBootstrapOut(
        ...
        seasons=list_seasons(competition_id=None, db=db, current_profile=current_profile),
        matchdays=list_matchdays(status=None, db=db, current_profile=current_profile),
        active_matchdays=list_matchdays(status=MatchdayStatus.ACTIVE, db=db, current_profile=current_profile),
